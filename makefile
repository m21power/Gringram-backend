...
startmigration:
	docker run -it --rm --network host --volume "$(shell pwd)/db:/db" migrate/migrate:v4.17.0 create -ext sql -dir /db/migrations -seq comment_parentId
...
createdb:
	docker exec -it gringram mysql -uroot -pabate -e "CREATE DATABASE gringram;"
